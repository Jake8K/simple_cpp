 ...
 ** Description: Creates a Gollum with Gollum's special attributes
 ** Parameters: None
 ** Pre-Conditions: None
...
int Gollum::attack() {
    ...
    if (ring == 5) {
         hit = rollMyDice(3, 6);
         cout << "Gollum snuck up with his ring... \n";
    }
    else
        hit = rollMyDice(attackDice, attackDsides);
    
    cout << "Gollum strikes with " << hit << " attack points!\n";
...
void Gollum::defend(int hit) {
    ...
    if (damage > 0)
        cout << "Gollum takes on " << damage << " damage! His strength is now "
                                             << this->strength << "!\n";
    else if (damage <= 0)
        cout << "Gollum escaped the attack!\n";
